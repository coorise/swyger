    ...
    try {
        if(data?.path && data?.uid){
            ...
            if(result?.error){
                ...
                if(result?.data?.name && data?.newPath){
                    const newUploadPath=storageDir+data?.newPath+'/'+data?.uid
                    const oldUploadPath=storageDir+data?.path+'/'+data?.uid
                    //console.log('file path: ',glob.sync(uploadPath+'.*',{dot: true})?.[0])
                    const file=oldUploadPath+result?.data?.extension
                    const extension=result?.data?.extension
                    if(fs.pathExistsSync(file))
                    fs.moveSync(file,newUploadPath+extension,{overwrite:true})
                }
