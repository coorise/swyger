    ...
    if(create?.name){
        if(!specialChars.test(create.name)){
            ...
            try {
                ...
                let db = new JsonDB(new Config(base+'/app/api_builder/typeorm/'+PluralName+'.builder.json', true, true, '/'));
...
const findOne=(entities={},data)=>{
    ...
    if(fs.existsSync(path.join(__dirname,'../../../../','api_builder/typeorm',filename))){
        try {
            let db = new JsonDB(new Config(base+'/app/api_builder/typeorm/'+filename, true, true, '/'));
            //console.log('get data ',data)
            model=db.getData('/')
        }catch (e) {
...
const findMany=(entities={},option={})=>{
    ...
    let getFiles=(
        data=[
            base+'/app/api_builder/typeorm/**/'+option.name+'.builder.json',
            base+'/app/api_builder/typeorm/**/'+option.name+'.builder.json',
        ]
    )=> {
        return glob.sync(
...
const updateOne=(entities={},create)=>{
    if(!fs.existsSync(path.join(__dirname,'../../../../','api_builder/typeorm',create?.filename))){
        error.errors.push('Error with writing: '+error.toString())
        if(error.errors.length>0){
            response.error=error
        }
...
const deleteOne=(entities={},data)=>{
    ...
    if(fs.existsSync(path.join(__dirname,'../../../../','api_builder/typeorm',filename))){
        try {
            fs.unlinkSync(path.join(__dirname,'../../../../','api_builder/typeorm',filename))
        }catch (e) {
            error.errors.push('Error with deleting: '+error.toString())
        }
    }else {
        error.errors.push('File doesn\'t exist: '+filename)
    }
    response.data='ok'
    if(error.errors.length>0){
        response.error=error
