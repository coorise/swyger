...
export default class DocModelRoute{
    ...
    setSendOne(data=this.setData.send.one.data){
        let path
        if(!data?.path){
            path=this.parentPath+'/send-one'
        } else{
            path=data.path
        }
        let defaultValue={
    ...
    setSendMany(data=this.setData.send.many.data){
        let path
        if(!data?.path){
            path=this.parentPath+'/send-many'
        } else{
            path=data.path
        }
        let defaultValue={
            ...
            description:'Send many items',
            parameter:{
                description:'Param has items in array[item,item2,...]',
                schema:{
                    "type": "array",
                    "items":{
                        ...this.setData.send.one.data?.parameter?.schema
                    }
    ...
    getDefaultCRUD(){
        ...
        if(this.setData.send.one.isActive){

            this.setSendOne()
        }

        if(this.setData.send.many.isActive){
            this.setSendMany()
        }
