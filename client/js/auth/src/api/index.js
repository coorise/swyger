...
class SwygerClient{
  ...
  init=(req)=>{
    ...
    req?.socket?.auth?.on('connect',()=>{
      //console.log('device connected')
      this.#dataBaseConnected=true
      offlineDatabaseHandler()
    })
    req?.socket?.auth?.on('disconnect',()=>{
    ...
    const listenEmitter=(ref,callback=()=>{})=>{
      ...
      let eventName=API_VERSION?.AUTH+listenLocal+ref
      eventName=eventName.replace(/\/\//g,'/')
    ...
    let localDatabase=(method,path,data={},node,callback)=>{
      ...
      let handlerRequest=async ()=>{
        ...
        const listenLocal=this.#listen
        let eventName=API_VERSION?.AUTH+listenLocal+path+'/'+method
    ...
    return {
      ...
      ref:(path='',config={
        host:HOST_SERVER.AUTH,
        version:API_VERSION.AUTH,
        base:authRoute.base
      },socket=req?.socket?.socket,requireAuth=true)=>{
        path=path.trim()
        path=path.replace(/\/\//g,'/')
        ...
        return {
          ...
          create: (...args) => {
            ...
            if (parent.auth().$currentUserId && path.includes('/users') && this.#syncWithLocal && !this.#dataBaseConnected) {
